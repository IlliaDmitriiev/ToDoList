...
std::vector<std::weak_ptr<FullTask>> ViewByPriority::getTasksForToday(boost::gregorian::date date) {
    std::vector<std::weak_ptr<FullTask>> v;
    return v;
}

std::vector<std::weak_ptr<FullTask>> ViewByPriority::getTasksForWeek(boost::gregorian::date date){
    std::vector<std::weak_ptr<FullTask>> v;
    return v;
}
