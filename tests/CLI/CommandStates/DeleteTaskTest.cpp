...
class DeleteTaskTest : public ::testing::Test {
public:
    DeleteTaskCommand state_;
    void SetUp() override{
    }
};

TEST_F(DeleteTaskTest, shouldRead1) {
    auto io = std::make_unique<MockConsoleIO>();
    EXPECT_EQ(state_.read(*io), CommandState::Type::Option);
}

TEST_F(DeleteTaskTest, shouldRead2) {
    auto io = std::make_unique<MockConsoleIO>();
    EXPECT_NE(state_.read(*io), CommandState::Type::DeleteTask);
}


TEST_F(DeleteTaskTest, shouldPrint) {
    auto io = std::make_unique<MockConsoleIO>();
    EXPECT_NO_THROW(state_.print(*io));
}


TEST_F(DeleteTaskTest, shouldChange1) {
    auto io = std::make_unique<MockConsoleIO>();
...
TEST_F(DeleteTaskTest, shouldChange2) {
    auto io = std::make_unique<MockConsoleIO>();
    auto command_map = CommandMap::create();
